...
os.system(f"cp {homedir}/.aliases_added {homedir}/.ota_markers/.aliases_added > /dev/null 2>&1")
if not os.path.exists(homedir+"/.ota_markers/.aliases_added"):
    os.system(f"cp {homedir}/.updater_self {homedir}/.ota_markers/.updater_self >/dev/null 2>&1")
if not os.path.exists(homedir+"/.ota_markers/.installation-check_file.txt"):
    os.system(f"cp {homedir}/.old_RotorHazard.old/.installation-check_file.txt {homedir}/.ota_markers/.installation-check_file.txt > /dev/null 2>&1")
os.system(f"cp {homedir}/.serialok {homedir}/.ota_markers/.serialok > /dev/null 2>&1")
if os.path.exists(homedir+"/.bashrc"):         # aliases compatibility
    if check_if_string_in_file(homedir+'/.bashrc', 'RotorHazard OTA Manager updated'):
        os.system(f"sed -i 's/alias updateupdater/# alias updateupdater/g' {homedir}/.bashrc")
        os.system(f"sed -i 's/RotorHazard OTA Manager updated/old alias/g' {homedir}/.bashrc")
    ...
    if check_if_string_in_file(homedir+'/.bashrc', 'starts the server'):
        os.system(f"sed -i 's/alias ss/# alias ss/g' {homedir}/.bashrc")
        os.system(f"sed -i 's/starts the server/old alias/g' {homedir}/.bashrc")
    ...
    if os.path.exists("./updater-config.json"):         # json compatibility
        if not check_if_string_in_file(homedir+'/RH-ota/updater-config.json', '"pins_assignment"'):
            os.system(f"sed -i 's/\"debug_mode\" : 0/\"debug_mode\" : 0,/g' {homedir}/RH-ota/updater-config.json")
            os.system(f"sed -i 's/\"debug_mode\" : 1/\"debug_mode\" : 1,/g' {homedir}/RH-ota/updater-config.json")
        ...
        if check_if_string_in_file(homedir+'/RH-ota/updater-config.json', '"pins_assignment"'):
            if not check_if_string_in_file(homedir+'/RH-ota/updater-config.json', '"updates_without_pdf"'):
                os.system(f"sed -i 's/\"pins_assignment\" : \"default\"/\"pins_assignment\" : \"default\",/g' {homedir}/RH-ota/updater-config.json")
                os.system(f"sed -i 's/\"pins_assignment\" : \"custom\"/\"pins_assignment\" : \"custom\",/g' {homedir}/RH-ota/updater-config.json")
                os.system(f"sed -i 's/\"pins_assignment\" : \"PCB\"/\"pins_assignment\" : \"PCB\",/g' {homedir}/RH-ota/updater-config.json")
...
if not check_if_string_in_file(homedir+'/.ota_markers/ota_config.txt', 'curl_installed'):
    os.system(f"echo curl_installed = 0 | tee -a {homedir}/.ota_markers/ota_config.txt > /dev/null ")
