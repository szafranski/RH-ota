...
def flash_blink_onto_a_node(config):
    os.system(f"avrdude -v -p atmega328p -c arduino -P /dev/{config.port_name} -b 115200 -U \
...
def flash_firmware_onto_all_nodes(config):  # nodes have to be 'auto-numbered'
    ...
    if nodes_num == 1:
        ...
        for i in range(0, nodes_num):
            ...
            print(f"avrdude -v -p atmega328p -c arduino -P /dev/{config.port_name} -b 115200 -U "
    ...
    def flash_node_first_time(port):
        ...
        while True:
            ...
            if selection == 'e':
                ...
                sleep(0.5)
                os.system(f"timeout 10 avrdude -v -p atmega328p -c arduino -P /dev/{port} -b 115200 -U \
                flash:w:/home/{config.user}/RH-ota/firmware/{firmware_version_selection(config)}/node_0.hex:i \
                || {show_flash_error_msg()}")
...
def check_uart_connection(config):
    os.system(f"avrdude -v -p atmega328p -c arduino -P /dev/{config.port_name} -b 115200 || {show_uart_con_error_msg()}")
...
def check_uart_con_with_a_node(config, selected_node_number):
    ...
    prepare_mate_node(addr) if not config.debug_mode else print("simulation mode - flashing disabled")
    print(f"avrdude -v -p atmega328p -c arduino -P /dev/{config.port_name} -b 115200")
